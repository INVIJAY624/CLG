...
int main()
{
int t[20], d[20], h, i, j, n, temp, k, atr[20], tot, p, sum=0; printf("Enter the no of tracks to be Traversed : "); scanf("%d'",&n);
printf("\nEnter the position of Head: ");
scanf("%d",&h);
t[0]=0;
t[1]=h;
printf("\nEnter the Tracks: "); for(i=2;i<n+2;i++) scanf("%d",&t[i]); for(i=0;i<n+2;i++)
{
for(j=0;j<(n+2)-i-1;j++) {
if(t[j]>t[j+1]) {
